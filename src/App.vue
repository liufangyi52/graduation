...
const ragIndexReady = computed(() => Boolean(ragIndexStatus.value?.configured && ragIndexStatus.value.ready))
...
async function loadExperimentSummary() {
  ...
  try {
    const [summary, status] = await Promise.all([meetings.experimentSummary(experimentProjectId.value), meetings.ragIndexStatus(experimentProjectId.value)])
...
async function syncRagIndex() {
  ...
  experimentNotice.value = ''
  try {
    const result = await meetings.syncRagIndex(experimentProjectId.value)
    experimentNotice.value = `RAG 索引同步完成：${result.indexedChunks} 个脱敏分块`
    ragIndexStatus.value = await meetings.ragIndexStatus(experimentProjectId.value)
  } catch (reason) {
    experimentNotice.value = reason instanceof Error ? reason.message : 'RAG 索引同步失败'
  } finally { ragIndexSyncing.value = false }
}
    ...
    <main class="main-canvas" :class="{ expanded: !sidebarOpen }">
      ...
      <div class="content">
        ...
        <section v-else-if="currentPage === 'my-tasks'" class="page-section member-workspace">
          ...
          <div class="member-task-grid"><article v-for="task in myTasks" :key="task.id" class="member-task-card"><div class="member-card-head"><span class="tag" :class="taskStatusLabel(task.state).tone">{{ taskStatusLabel(task.state).label }}</span><span class="mono">{{ task.id }}</span></div><h3>{{ task.title }}</h3><p>{{ task.project }}</p><div class="member-task-meta"><span><CalendarDays :size="14" />{{ task.due }}</span><span class="tag" :class="priorityLabel(task.rawPriority || task.priority).tone">{{ priorityLabel(task.rawPriority || task.priority).label }}</span></div><div class="member-progress"><div><span>当前进度</span><b>{{ task.progress }}%</b></div><div class="progress-track"><span :style="{ width: `${task.progress}%` }"></span></div></div><div class="member-card-actions"><button class="secondary-button" @click="openFeedback(task)"><ClipboardList :size="15" /> 提交反馈</button><button v-if="task.state !== 'completed'" class="primary-button" @click="updateTask(task, task.state === 'todo' ? 'in-progress' : 'completed')">{{ task.state === 'todo' ? '开始任务' : '完成任务' }}</button></div></article><div v-if="!myTasks.length" class="empty-member panel"><CheckCircle2 :size="26" /><strong>当前没有分配给你的任务</strong><span>任务分派后会自动出现在这里。</span></div></div>
