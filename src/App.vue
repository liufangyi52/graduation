...
import {
  ...
  ChevronDown, CircleHelp, ClipboardList, FolderKanban, Gauge, LayoutDashboard, Menu,
  Moon, MoreHorizontal, PanelLeft, Plus, Search, Settings, ShieldAlert, Sparkles,
  Sun, Target, TestTube2, Users, X, Zap,
} from 'lucide-vue-next'
import { createWorkspaceService, type Project, type RiskLevel, type Task, type TaskState } from './services/workspaceService'
...
if (props.user.role === 'admin') void auth.listUsers(props.token).then((items) => { managedUsers.value = items }).catch(() => {})
...
onMounted(() => {
  dateTimer = window.setInterval(() => { todayLabel.value = formatLocalDate(new Date()) }, 60_000)
})
...
async function submitMeeting() {
  ...
  try {
    const meeting = await meetings.create(meetingProjectId.value, meetingTitle.value.trim(), meetingContent.value.trim())
...
async function reviewMeetingAnalysis(id: string, approved: boolean) {
  try { await meetings.review(id, approved); await meetings.load(); flash(approved ? '分析已通过并生成任务' : '分析已驳回') } catch (reason) { flash(reason instanceof Error ? reason.message : '审核失败') }
}
...
<template>
  <div class="app-shell" :class="{ dark }">
    <aside class="sidebar" :class="{ collapsed: !sidebarOpen }">
      ...
      <button v-if="sidebarOpen && (user.role === 'manager' || user.role === 'admin')" class="new-project-button" @click="showCreateProject = true"><Plus :size="16" /> 新建项目</button>
      <div class="sidebar-bottom">
        ...
        <button class="nav-item" title="帮助中心" @click="flash('帮助中心即将上线')"><CircleHelp :size="18" /><span v-if="sidebarOpen">帮助中心</span></button>
      ...
      <div class="content">
        <div class="page-intro" v-if="currentPage !== 'dashboard'">
          ...
          <div class="page-actions">
            <button v-if="currentPage === 'projects'" class="primary-button" @click="showCreateProject = true"><Plus :size="15" /> 新建项目</button>
            <button v-if="currentPage === 'tasks'" class="primary-button" @click="flash('任务创建表单已打开')"><Plus :size="15" /> 新建任务</button>
        ...
        <section v-if="currentPage === 'dashboard' && !isMember" class="page-section">
          <div class="welcome-row"><div><p class="eyebrow">{{ todayLabel }}</p><h2>下午好，{{ user.name }}</h2><p class="muted">今天有 {{ pendingReviews.length }} 条 AI 结果待审核，{{ highRiskCount }} 个高风险项目需要关注。</p></div><button class="primary-button" @click="navigate('/projects')"><FolderKanban :size="16" /> 查看项目</button></div>
          <div class="metric-grid">
            <article class="metric-card"><div class="metric-label">项目完成率 <Target :size="16" /></div><div class="metric-number">68<span>%</span></div><div class="progress-track"><span style="width:68%"></span></div><small>较上周 <b class="success-text">+8.4%</b></small></article>
        ...
        <section v-else-if="currentPage === 'dashboard' && isMember" class="page-section member-dashboard">
          <div class="welcome-row"><div><p class="eyebrow">MEMBER WORKSPACE · {{ roleLabels[user.role] }}</p><h2>下午好，{{ user.name }}</h2><p class="muted">这里仅展示与你有关的任务、工作反馈与通知。</p></div><button class="primary-button" @click="navigate('/my-tasks')"><ClipboardList :size="16" /> 查看我的任务</button></div>
        ...
        <section v-else-if="currentPage === 'calendar'" class="page-section"><div class="calendar-toolbar"><button class="icon-button" disabled><ArrowRight :size="16" class="rotate-180" /></button><strong>团队日历</strong><button class="icon-button" disabled><ArrowRight :size="16" /></button><button class="secondary-button" disabled>今天</button></div><article class="panel empty-cell">暂无已持久化的会议或任务日程。</article></section>
        ...
        <section v-else-if="currentPage === 'settings'" class="page-section"><div class="settings-layout"><div class="settings-nav panel"><button class="selected">模型与运行模式</button><button>数据脱敏规则</button><button>账号与角色</button><button>通知设置</button></div><article class="panel settings-content"><div class="panel-heading"><div><p class="eyebrow">MODEL PROVIDER</p><h3>模型与运行模式</h3></div><span class="tag green">已保存</span></div><label class="setting-field"><span>默认模型</span><select v-model="data.settings.model"><option>DeepSeek V3</option><option>Qwen 2.5</option><option>GPT-4o</option></select></label><label class="setting-field"><span>AI 运行模式</span><select v-model="data.settings.mode"><option>无 AI</option><option>单轮大模型</option><option>RAG 检索增强</option><option>智能体编排</option></select></label><div class="setting-toggle"><div><strong>会议内容自动脱敏</strong><small>对手机号、邮箱和身份证号进行替换处理</small></div><button class="toggle" :class="{ on: data.settings.desensitize }" @click="data.settings.desensitize = !data.settings.desensitize"><i></i></button></div><button class="primary-button" @click="flash('设置已保存')">保存设置</button></article></div></section>
        <section v-else-if="currentPage === 'meetings'" class="page-section"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">MEETING MINUTES</p><h3>提交会议纪要并发起 DeepSeek 分析</h3></div></div><label class="modal-field"><span>所属项目</span><select v-model="meetingProjectId"><option value="">请选择项目</option><option v-for="project in data.projects" :key="project.id" :value="project.id">{{ project.name }}</option></select></label><label class="modal-field"><span>会议标题</span><input v-model="meetingTitle" placeholder="例如：版本发布评审会" /></label><label class="modal-field"><span>会议纪要</span><textarea v-model="meetingContent" rows="12" placeholder="粘贴会议纪要内容，DeepSeek 将抽取任务、决策和风险"></textarea></label><p v-if="meetingNotice" class="muted">{{ meetingNotice }}</p><button class="primary-button" @click="submitMeeting"><Zap :size="15" /> 提交并分析</button></article></section>
        ...
        <section v-else-if="currentPage === 'users'" class="page-section"><article class="panel table-panel"><div class="panel-heading"><div><p class="eyebrow">ACCOUNT ADMINISTRATION</p><h3>账号与角色</h3></div></div><div class="table-wrap"><table><thead><tr><th>姓名</th><th>邮箱</th><th>角色</th><th>状态</th></tr></thead><tbody><tr v-for="managed in managedUsers" :key="managed.id"><td>{{ managed.name }}</td><td>{{ managed.email }}</td><td><select :value="managed.role" @change="auth.updateUser(props.token, managed.id, { role: ($event.target as HTMLSelectElement).value as UserRole }).then(() => managed.role = ($event.target as HTMLSelectElement).value as UserRole)"><option value="manager">项目经理</option><option value="member">项目成员</option><option value="admin">系统管理员</option><option value="auditor">审计员</option></select></td><td>{{ managed.is_active ? '启用' : '停用' }}</td></tr></tbody></table></div></article></section>
