...
import { beijingGreeting, formatBeijingDate, formatBeijingMinute } from './utils/date'
...
import { createDashboardRealtimeService } from './services/dashboardRealtimeService'
import { buildProjectAnalytics } from './utils/projectAnalytics'
import { averageDeliveryCycleDays, buildMemberDeliveryEfficiency } from './utils/projectAnalytics'
...
function refreshDashboardData() {
  ...
  dashboardRefresh = (async () => {
    do {
      ...
      await service.load()
      dashboardRealtime?.syncProjects(data.projects.map((project) => project.id))
...
if (props.user.role === 'auditor') {
  ...
  void refreshDashboardData().catch(() => flash('真实数据加载失败，请检查后端服务'))
...
const notificationDraft = ref({ title: '', body: '', audienceType: 'user' as 'user' | 'role', userId: '', role: 'member' as UserRole })
const notificationSending = ref(false)
const notificationSendError = ref('')
...
onMounted(() => {
  ...
  if (!isAuditor.value && props.user.role !== 'manager') taskRefreshTimer = window.setInterval(() => { void refreshDashboardData().catch(() => {}) }, 10_000)
...
const efficiencyTasks = computed(() => data.tasks.map((task) => ({
  owner: task.owner,
  status: task.state,
  progress: task.progress,
  createdAt: task.createdAt,
...
const memberDeliveryEfficiency = computed(() => buildMemberDeliveryEfficiency(efficiencyTasks.value)
  .map((member, index) => ({ ...member, rank: index + 1 })))
const averageDeliveryCycle = computed(() => averageDeliveryCycleDays(efficiencyTasks.value))
...
const myTaskCompletionRate = computed(() => averageTaskProgress(myTasks.value))
...
async function sendNotification() {
  const title = notificationDraft.value.title.trim()
  const body = notificationDraft.value.body.trim()
  const isUserAudience = notificationDraft.value.audienceType === 'user'
  if (!title || !body || (isUserAudience && !notificationDraft.value.userId)) {
    notificationSendError.value = '请填写通知标题、正文和接收对象'
    return
  }
  notificationSending.value = true
  notificationSendError.value = ''
  try {
    const input = isUserAudience
      ? { title, body, audienceType: 'user' as const, userId: notificationDraft.value.userId }
      : { title, body, audienceType: 'role' as const, role: notificationDraft.value.role }
    const result = await service.sendNotification(input)
    notificationDraft.value = { title: '', body: '', audienceType: 'user', userId: '', role: 'member' }
    await service.loadNotifications()
    flash(`通知已发送给 ${result.created} 名账号`)
  } catch (reason) {
    notificationSendError.value = reason instanceof Error ? reason.message : '通知发送失败'
  } finally {
...
function resetCreateUserForm() {
  ...
  roleDraft.value = 'member'
  createUserErrors.value = { name: '', email: '', password: '' }
...
<template>
  <div class="app-shell" :class="{ dark }">
      ...
      <div class="content">
        ...
        <section v-if="currentPage === 'dashboard' && canManageBusiness" class="page-section">
          ...
          <div class="dashboard-grid top-grid">
            <article class="panel review-panel"><div class="panel-heading"><div><p class="eyebrow">AI REVIEW QUEUE</p><h3>待审核 AI 分析任务</h3></div><button class="text-button" @click="navigate('/reviews')">查看全部 <ArrowRight :size="14" /></button></div><div class="table-wrap"><table><thead><tr><th>会议标题</th><th>分析模式</th><th>置信度</th><th>时间</th><th></th></tr></thead><tbody><tr v-for="review in pendingReviews" :key="review.id"><td><strong>{{ review.meeting }}</strong><small>{{ review.project }}</small></td><td><span class="tag blue">{{ review.mode }}</span></td><td><div class="confidence"><span><i :style="{ width: `${review.confidence}%` }"></i></span><b>{{ review.confidence }}%</b></div></td><td class="mono">{{ formatBeijingMinute(review.time) }}</td><td><button class="small-button" @click="reviewMeetingAnalysis(review.id, true)">审核</button></td></tr><tr v-if="!pendingReviews.length"><td colspan="5" class="empty-cell">暂无待审核记录</td></tr></tbody></table></div></article>
            <article class="panel due-panel"><div class="panel-heading"><div><p class="eyebrow">DEADLINE WATCH</p><h3>即将到期任务</h3></div><CalendarDays :size="18" class="panel-icon" /></div><div class="due-list"><div v-for="task in dueTasks" :key="task.id" class="due-item" :class="priorityLabel(task.rawPriority || task.priority).tone === 'red' ? 'critical' : priorityLabel(task.rawPriority || task.priority).tone === 'amber' ? 'high' : 'normal'"><div><strong>{{ task.title }}</strong><div class="due-meta"><span class="avatar small">{{ task.owner.slice(0, 1) }}</span>{{ task.owner }}<span class="mono">{{ formatBeijingMinute(task.due) }}</span></div></div><span class="tag" :class="priorityLabel(task.rawPriority || task.priority).tone">{{ priorityLabel(task.rawPriority || task.priority).label }}</span></div><p v-if="!dueTasks.length" class="due-empty">暂无即将到期任务</p></div><button class="wide-ghost" @click="navigate('/tasks')">查看完整任务表 <ArrowRight :size="15" /></button></article>
          </div>
          <div class="dashboard-grid bottom-grid">
            <article class="panel chart-panel">
              <div class="panel-heading"><div><p class="eyebrow">DELIVERY TREND</p><h3>任务完成趋势</h3></div><span class="chart-range">实时数据</span></div>
            ...
            <article class="panel risk-panel"><div class="panel-heading"><div><p class="eyebrow">RISK DISTRIBUTION</p><h3>风险分布统计</h3></div><AlertTriangle :size="18" class="panel-icon" /></div><div class="donut-wrap"><div class="donut" :style="riskDonutStyle"><div><strong>{{ data.risks.length }}</strong><span>总风险项</span></div></div></div><div class="risk-legend"><div><span><i class="dot red-dot"></i>高风险</span><b>{{ data.risks.filter((risk) => risk.level === 'high').length }}</b></div><div><span><i class="dot amber-dot"></i>中风险</span><b>{{ data.risks.filter((risk) => risk.level === 'medium').length }}</b></div><div><span><i class="dot green-dot"></i>低风险</span><b>{{ data.risks.filter((risk) => risk.level === 'low').length }}</b></div></div></article>
          </div>
        </section>
        ...
        <section v-else-if="currentPage === 'dashboard' && user.role === 'admin'" class="page-section">
          ...
          <article class="panel table-panel"><div class="panel-heading"><div><p class="eyebrow">RECENT AUDIT EVENTS</p><h3>最近审计记录</h3></div><button class="text-button" @click="navigate('/audit-logs')">查看全部 <ArrowRight :size="14" /></button></div><div class="table-wrap"><table><thead><tr><th>时间</th><th>操作者</th><th>操作</th><th>对象</th></tr></thead><tbody><tr v-for="log in auditLogs.slice(0, 5)" :key="log.id"><td class="mono">{{ formatBeijingMinute(log.created_at) }}</td><td>{{ log.actor_name ?? '系统' }}</td><td>{{ auditActionLabel(log.action) }}</td><td>{{ auditLogEntityLabel(log.entity_type, log.entity_id) }}</td></tr><tr v-if="!auditLogs.length"><td colspan="4" class="empty-cell">暂无审计记录</td></tr></tbody></table></div></article>
        </section>
        ...
        <section v-else-if="currentPage === 'efficiency'" class="page-section efficiency-page">
          <div class="metric-grid compact">
            <article class="metric-card"><div class="metric-label">已完成任务 <Check :size="16" /></div><div class="metric-number">{{ completedCount }}</div><small>实时汇总当前项目任务</small></article>
            <article class="metric-card"><div class="metric-label">平均交付周期 <Gauge :size="16" /></div><div class="metric-number">{{ averageDeliveryCycle === null ? '—' : `${averageDeliveryCycle.toFixed(1)}天` }}</div><small>{{ averageDeliveryCycle === null ? '任务记录暂无完整周期数据' : '按已完成任务的实际时间计算' }}</small></article>
            <article class="metric-card"><div class="metric-label">活跃成员 <Users :size="16" /></div><div class="metric-number">{{ activeMemberCount }}<span>人</span></div><small>按当前任务负责人统计</small></article>
          </div>
          <div class="analysis-grid">
            <article class="panel chart-panel efficiency-panel">
              <div class="panel-heading"><div><p class="eyebrow">TEAM DELIVERY</p><h3>团队完成趋势</h3></div><span class="chart-range">实时数据</span></div>
              <div v-if="dashboardCompletionTrend.length" class="efficiency-trend" aria-label="团队完成趋势">
                <div v-for="point in dashboardCompletionTrend" :key="point.date" class="efficiency-trend-row">
                  <span class="mono">{{ point.date }}</span>
                  <span class="efficiency-trend-track"><i :style="{ width: `${point.count / dashboardTrendMaximum * 100}%` }"></i></span>
                  <b>{{ point.count }}</b>
                </div>
              </div>
              <div v-else class="empty-cell efficiency-empty">暂无可追溯的任务完成记录</div>
              <p class="chart-note">成员完成任务后将立即同步更新。</p>
            </article>
            <article class="panel member-panel efficiency-panel">
              <div class="panel-heading"><div><p class="eyebrow">TEAM LEADERBOARD</p><h3>成员交付效率</h3></div><span class="chart-range">{{ memberDeliveryEfficiency.length }} 人</span></div>
              <div v-if="memberDeliveryEfficiency.length" class="efficiency-member-list">
                <div v-for="member in memberDeliveryEfficiency" :key="member.owner" class="efficiency-member-row">
                  <span class="rank">{{ member.rank }}</span>
                  <span class="avatar efficiency-member-avatar">{{ member.owner.slice(0, 1) }}</span>
                  <div class="efficiency-member-main">
                    <div class="efficiency-member-meta"><strong>{{ member.owner }}</strong><span>{{ member.completedTasks }} / {{ member.totalTasks }} 完成</span></div>
                    <span class="efficiency-progress-track"><i :style="{ width: `${member.averageProgress}%` }"></i></span>
                    <small>平均进度 {{ member.averageProgress }}%</small>
                  </div>
                  <div class="efficiency-member-result"><b>{{ member.completionRate }}%</b><span>完成率</span></div>
                </div>
              </div>
              <div v-else class="empty-cell efficiency-empty">暂无可统计的成员任务。</div>
            </article>
          </div>
        </section>
        ...
        <section v-else-if="currentPage === 'settings'" class="page-section">
          <article class="panel settings-content">
            ...
            <label class="setting-field"><span>默认模型</span><select v-model="systemSettings.model"><option value="deepseek-v4-pro">DeepSeek V4 Pro</option></select></label>
            <label class="setting-field"><span>AI 运行模式</span><select v-model="systemSettings.mode"><option value="manual">人工审核</option><option value="llm">单轮大模型</option><option value="rag">RAG 检索增强</option><option value="agent">智能体编排</option></select></label>
        ...
        <section v-else-if="currentPage === 'users'" class="page-section"><article class="panel table-panel"><div class="panel-heading"><div><p class="eyebrow">ACCOUNT ADMINISTRATION</p><h3>账号与角色</h3></div><button class="primary-button" @click="openCreateUser"><Plus :size="15" /> 新增账号</button></div><div class="table-wrap"><table><thead><tr><th>姓名</th><th>邮箱</th><th>角色</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="managed in managedUsers" :key="managed.id"><td>{{ managed.name }}</td><td>{{ managed.email }}</td><td><button v-if="editingManagedUserId !== managed.id" type="button" class="role-tag-button" :aria-label="`编辑角色：${systemRoleLabel(managed.role).label}`" @click="startManagedUserRoleEdit(managed.id)"><span class="tag" :class="systemRoleLabel(managed.role).tone">{{ systemRoleLabel(managed.role).label }}</span></button><select v-else :value="managed.role" aria-label="选择系统角色" @change="updateManagedUserRole(managed, ($event.target as HTMLSelectElement).value as UserRole)"><option value="manager">项目经理</option><option value="member">项目成员</option><option value="admin">系统管理员</option><option value="auditor">审计员</option></select></td><td><button class="small-button" @click="setManagedUserActive(managed, !managed.is_active)">{{ managed.is_active ? '停用' : '启用' }}</button></td><td><button class="small-button" @click="resetPasswordUser = managed; resetPasswordValue = ''">重置密码</button></td></tr></tbody></table></div></article></section>
        <section v-else-if="currentPage === 'audit-logs'" class="page-section"><article class="panel table-panel"><div class="panel-heading"><div><p class="eyebrow">AUDIT LOGS</p><h3>关键操作记录</h3></div></div><div class="table-wrap"><table><thead><tr><th>时间</th><th>操作者</th><th>操作</th><th>对象</th><th>详情</th></tr></thead><tbody><tr v-for="log in auditLogs" :key="log.id"><td class="mono">{{ formatBeijingMinute(log.created_at) }}</td><td>{{ log.actor_name ?? '系统' }}</td><td>{{ auditActionLabel(log.action) }}</td><td>{{ auditLogEntityLabel(log.entity_type, log.entity_id) }}</td><td>{{ auditLogDetailsLabel(log.details) }}</td></tr><tr v-if="!auditLogs.length"><td colspan="5" class="empty-cell">暂无审计记录</td></tr></tbody></table></div></article></section>
      </div>
    </main>
    ...
    <div v-if="showCreateUser" class="modal-backdrop" @click.self="closeCreateUser"><div class="modal"><div class="modal-head"><div><p class="eyebrow">ACCOUNT ADMINISTRATION</p><h3>新增账号</h3></div><button class="icon-button" :disabled="creatingUser" title="关闭" @click="closeCreateUser"><X :size="18" /></button></div><label class="modal-field"><span>姓名 <b>*</b></span><input v-model="newUserName" :aria-describedby="createUserErrors.name ? 'new-user-name-error' : undefined" :disabled="creatingUser" autofocus /><small v-if="createUserErrors.name" id="new-user-name-error" class="form-error">{{ createUserErrors.name }}</small></label><label class="modal-field"><span>邮箱 <b>*</b></span><input v-model="newUserEmail" type="email" :aria-describedby="createUserErrors.email ? 'new-user-email-error' : undefined" :disabled="creatingUser" /><small v-if="createUserErrors.email" id="new-user-email-error" class="form-error">{{ createUserErrors.email }}</small></label><label class="modal-field"><span>初始密码 <b>*</b></span><input v-model="newUserPassword" type="password" minlength="8" :aria-describedby="createUserErrors.password ? 'new-user-password-error' : undefined" :disabled="creatingUser" /><small v-if="createUserErrors.password" id="new-user-password-error" class="form-error">{{ createUserErrors.password }}</small></label><label class="modal-field"><span>角色 <b>*</b></span><select v-model="roleDraft" :disabled="creatingUser"><option value="manager">项目经理</option><option value="member">项目成员</option><option value="admin">系统管理员</option><option value="auditor">审计员</option></select></label><p v-if="createUserSubmitError" class="form-error" role="alert">{{ createUserSubmitError }}</p><div class="modal-actions"><button class="secondary-button" :disabled="creatingUser" @click="closeCreateUser">取消</button><button class="primary-button" :aria-busy="creatingUser" :disabled="creatingUser" @click="createManagedUser">{{ creatingUser ? '创建中...' : '创建账号' }}</button></div></div></div>
