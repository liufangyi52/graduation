...
import {
  ...
  Moon, MoreHorizontal, PanelLeft, Pencil, Plus, RotateCcw, Search, Settings, ShieldAlert, Sparkles, StickyNote, Tags, Trash2,
...
function openProjectEditor(project: Project) {
  ...
  projectDraft.value = { name: project.name, code: project.code, description: '', endDate: project.deadline === '未设置' ? '' : project.deadline, status: project.state === '暂停' ? 'paused' : project.state === '已归档' ? 'archived' : 'active' }
}
...
async function addDesensitizationRule() {
  ...
  try { await service.createDesensitizationRule(ruleProject.value.id, ruleDraft.value); desensitizationRules.value = await service.listDesensitizationRules(ruleProject.value.id); ruleDraft.value = { name: '', pattern: '', replacement: '', enabled: true } } catch (reason) { flash(reason instanceof Error ? reason.message : '规则保存失败') }
}
...
<template>
  <div class="app-shell" :class="{ dark }">
    ...
    <div v-if="projectEditor" class="modal-backdrop" @click.self="projectEditor = null"><div class="modal"><div class="modal-head"><h3>编辑项目</h3><button class="icon-button" @click="projectEditor = null"><X :size="18" /></button></div><label class="modal-field"><span>项目名称</span><input v-model="projectDraft.name" /></label><label class="modal-field"><span>项目编码</span><input v-model="projectDraft.code" /></label><label class="modal-field"><span>截止日期</span><input v-model="projectDraft.endDate" type="date" /></label><label class="modal-field"><span>状态</span><select v-model="projectDraft.status"><option value="active">进行中</option><option value="paused">暂停</option><option value="archived">已归档</option></select></label><div class="modal-actions"><button class="secondary-button" @click="projectEditor = null">取消</button><button class="primary-button" @click="saveProject">保存</button></div></div></div>
    ...
    <div v-if="ruleProject" class="modal-backdrop" @click.self="ruleProject = null"><div class="modal"><div class="modal-head"><h3>自定义脱敏规则</h3><button class="icon-button" @click="ruleProject = null"><X :size="18" /></button></div><label class="modal-field"><span>规则名称</span><input v-model="ruleDraft.name" /></label><label class="modal-field"><span>正则表达式</span><input v-model="ruleDraft.pattern" /></label><label class="modal-field"><span>替换文本</span><input v-model="ruleDraft.replacement" /></label><div class="modal-actions"><button class="secondary-button" @click="ruleProject = null">关闭</button><button class="primary-button" @click="addDesensitizationRule">保存规则</button></div></div></div>
