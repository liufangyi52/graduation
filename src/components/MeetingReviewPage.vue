...
<template>
  <section class="page-section review-detail-page">
    ...
    <template v-else-if="detail">
      <div class="review-header"><div><p class="eyebrow">AI REVIEW DETAIL</p><h2>{{ detail.meeting.title }}</h2><p class="muted">分析状态：<span class="tag" :class="analysisStatusLabel(detail.analysis.status).tone">{{ analysisStatusLabel(detail.analysis.status).label }}</span></p></div><div v-if="canManage" class="page-actions"><button v-if="detail.analysis.status === 'rejected'" class="secondary-button" @click="reanalyze">重新分析</button><button class="primary-button" @click="submit(true)">通过</button><button class="secondary-button" @click="showReason = true">驳回</button></div></div>
