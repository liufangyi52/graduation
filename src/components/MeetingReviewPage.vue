...
const detail = ref<ReviewDetail | null>(null); const loading = ref(true); const error = ref(''); const notice = ref(''); const reason = ref(''); const showReason = ref(false); const selected = ref<string[]>([]); const saving = ref(false); const reanalysisMode = ref<AnalysisMode>('llm')
...
<template>
  <section class="page-section review-detail-page">
    ...
    <template v-else-if="detail">
      <div class="review-header"><div><p class="eyebrow">AI REVIEW DETAIL</p><h2>{{ detail.meeting.title }}</h2><p class="muted">分析状态：<span class="tag" :class="analysisStatusLabel(detail.analysis.status).tone">{{ analysisStatusLabel(detail.analysis.status).label }}</span></p></div><div v-if="canManage" class="page-actions"><template v-if="detail.analysis.status === 'rejected'"><label class="modal-field"><span>重新分析模式</span><select v-model="reanalysisMode" aria-label="重新分析模式"><option value="manual">人工审核</option><option value="llm">LLM</option><option value="rag">RAG</option><option value="agent">智能体</option></select></label><button class="secondary-button" @click="reanalyze">重新分析</button></template><template v-else-if="detail.analysis.status === 'pending'"><button class="primary-button" @click="submit(true)">通过</button><button class="secondary-button" @click="showReason = true">驳回</button></template></div></div>
      ...
      <p v-if="detail.analysis.executionMetadata?.retrievalStatus === 'not_configured'" class="meeting-notice muted">妫€绱㈡湭閰嶇疆</p>
