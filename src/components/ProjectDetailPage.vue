...
const completionTrendPoints = computed(() => {
  const points = analytics.value?.completionTrend ?? []
  const max = Math.max(...points.map((point) => point.count), 1)
  return points.map((point, index) => `${(index / Math.max(1, points.length - 1)) * 100},${94 - Math.round((point.count / max) * 74)}`).join(' ')
})
    ...
    <template v-else-if="detail">
      ...
      <template v-else-if="activeTab === 'analytics'">
        ...
        <div class="analytics-visual-grid">
          <article class="panel detail-panel analytics-chart-card"><div class="panel-heading"><div><p class="eyebrow">DELIVERY TREND</p><h3>任务完成趋势</h3></div><span class="muted">按完成日期统计</span></div><div v-if="!analytics?.completionTrend.length" class="empty-cell">暂无可追溯的任务完成记录</div><div v-else class="completion-trend-chart" role="img" aria-label="任务完成趋势"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline :points="completionTrendPoints" /></svg><div class="completion-trend-axis"><span v-for="item in analytics.completionTrend" :key="item.date">{{ item.date.slice(5) }}</span></div></div></article>
        ...
        <article class="panel detail-panel analytics-gantt-card"><div class="panel-heading"><div><p class="eyebrow">TASK PLAN</p><h3>任务计划甘特图</h3></div><span v-if="analytics?.ganttRange" class="muted">{{ analytics.ganttRange.start }} 至 {{ analytics.ganttRange.end }}</span></div><div v-if="!analytics?.ganttRange" class="empty-cell">暂无可绘制的任务日期</div><div v-else class="analytics-gantt-scroll" role="img" :aria-label="`任务计划：${analytics.ganttRange.start} 至 ${analytics.ganttRange.end}`"><div class="analytics-gantt-header"><span>任务</span><div class="gantt-axis"><span>{{ analytics.ganttRange.start }}</span><span>{{ analytics.ganttRange.end }}</span></div><span>计划周期</span></div><div class="project-gantt"><div v-for="task in analytics.ganttTasks" :key="task.id" class="gantt-row"><span>{{ task.title }}</span><div class="gantt-track"><i v-if="ganttTodayLeft !== null" class="gantt-today" title="今天" :style="{ left: `${ganttTodayLeft}%` }"></i><i class="gantt-plan" :style="{ left: `${task.left}%`, width: `${task.width}%` }"></i><i v-if="task.progress > 0" class="gantt-bar" :class="task.status" :style="{ left: `${task.left}%`, width: `${task.width * task.progress / 100}%` }"></i></div><small>{{ task.start }} 至 {{ task.end }}</small></div></div></div><p v-if="analytics?.undatedTasks.length" class="muted">另有 {{ analytics.undatedTasks.length }} 个任务未设置有效截止日期。</p></article>
      </template>
