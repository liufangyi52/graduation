...
async function exportProject(kind: ExportKind, format: 'xlsx' | 'pdf') {
  ...
  try { await downloadProjectExport(await service.getProjectExportData(detail.value.project.id, kind), kind, format) } catch (reason) { error.value = reason instanceof Error ? reason.message : '导出失败' } finally { exportBusy.value = '' }
}
onMounted(load)
</script>
