...
const states: Array<{ id: TaskState; label: string }> = [{ id: 'todo', label: taskStatusLabel('todo').label }, { id: 'in-progress', label: taskStatusLabel('in-progress').label }, { id: 'completed', label: taskStatusLabel('completed').label }, { id: 'closed', label: taskStatusLabel('closed').label }]
function initFromQuery() { const q = route.query; for (const key of Object.keys(filters.value) as Array<keyof typeof filters.value>) filters.value[key] = String(q[key] ?? '') }
function syncQuery() { const query: Record<string, string> = {}; for (const [key, value] of Object.entries(filters.value)) if (value) query[key] = value; if (view.value !== 'table') query.view = view.value; router.replace({ query }) }
...
async function exportMyTasks() {
  ...
  try { const payload = await service.getProjectExportData(projectId, 'tasks'); await downloadProjectExport(payload, 'tasks', 'xlsx') } finally { exportBusy.value = false }
}
...
<template>
  <section class="page-section task-board-page">
    ...
    <div v-else class="kanban-grid"><article v-for="state in states" :key="state.id" class="panel kanban-column"><div class="panel-heading"><h3>{{ state.label }}</h3><span class="tag gray">{{ columns[state.id].length }}</span></div><div v-for="task in columns[state.id]" :key="task.id" class="kanban-card"><strong>{{ task.title }}</strong><small>{{ task.project }} · {{ task.owner }}</small><span class="tag" :class="priorityLabel(task.rawPriority || task.priority).tone">{{ priorityLabel(task.rawPriority || task.priority).label }}</span><button v-if="canUpdate" class="small-button" @click="updateStatus(task)">推进</button></div><div v-if="!columns[state.id].length" class="empty-cell">暂无任务</div></article></div>
  </section>
</template>
