...
export function createMeetingService(token: string) {
  ...
  return {
    ...
    async analyze(id: string, mode?: AnalysisMode) { return request<any>(`/meetings/${id}/analyze`, { method: 'POST', body: JSON.stringify(mode ? { mode } : {}) }) },
    ...
    async reanalyze(id: string, mode?: AnalysisMode) { return request<any>(`/analyses/${id}/reanalyze`, { method: 'POST', body: JSON.stringify(mode ? { mode } : {}) }) },
    async analysisSettings() { return request<{ model: string; mode: AnalysisMode }>('/analysis-settings') },
