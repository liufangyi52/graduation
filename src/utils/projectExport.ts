...
export async function downloadProjectExport(payload: ExportPayload, kind: ExportKind, format: ExportFormat, date = new Date().toISOString().slice(0, 10)) {
  ...
  if (format === 'xlsx') {
    const columnNames = data.length ? Object.keys(data[0]) : ['璇存槑']
    const columns = columnNames.map((header) => ({ header, cell: (row: Record<string, unknown>) => ({ value: text(row[header]), type: String }) }))
    const workbook = writeXlsxFile(data.length ? data : [{ 璇存槑: '鏆傛棤鏁版嵁' }], { columns, sheet: labels[kind] })
